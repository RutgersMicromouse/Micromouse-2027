...
class Robot(object):
    ...
    def log_location(self, beacon_detected=False):
    ...
    def find_shortest_path(self):
        ...
        # --- DYNAMIC GOAL LOGIC START ---
        # Instead of the old center-grid logic, use the coordinates 
        # captured during the explore phase.
        if self.goal_coords is not None:
            goal_room = [self.goal_coords]
        else:
            # Fallback in case exploration ended without finding the target
            # Defaulting to center to avoid a crash
            goal_room = [[self.maze_dim // 2, self.maze_dim // 2]]
        # --- DYNAMIC GOAL LOGIC END ---
